...
def find_measures_arrays(
    ...
    done, ds = False, num_imgs_or_batches
    measure_values = {}
        ...
        try:
            for measure_name in measure_names:
                if measure_name not in measure_values:
                    val = []
                    for inputs_batch, targets_batch in zip(torch.chunk(inputs, ds), torch.chunk(targets, ds)):
                        val.append(measures.calc_measure(
                            measure_name,
                            net_orig,
                            device,
                            inputs_batch,
...
def find_measures(
    ...
    for k, v in measures_arr.items():
        if k == "jacov" or k == 'epe_nas' or k=='nwot' or k=='zen' or k=='vkdnw':
