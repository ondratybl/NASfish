...
class GRAF:
    def __init__(self, features, benchmark, dataloader=None, cached_data=None, cache_zcp_scores=True,
                 cache_features=True, compute_new_zcps=False, no_zcp_raise=False, no_feature_raise=False, num_batches=1):
    ...
    def _zcp_predictor(self, name):
        if name not in self.zcp_predictors:
            self.zcp_predictors[name] = get_zcp_predictor(name, self.num_batches)
                ...
                if self.cache_zcp_scores:
                    if isinstance(result, dict):
                        for k, v in result.items():
                            self._cache_score(net.get_hash(), k, v)
                    else:
